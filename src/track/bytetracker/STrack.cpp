...
void STrack::activate(byte_kalman::KalmanFilter &kalman_filter, int frame_id, float time)
{
...
void STrack::re_activate(STrack &new_track, int frame_id, float time, bool new_id)
{
...
void STrack::update(STrack &new_track, int frame_id, float time)
{
	this->frame_id = frame_id;
	this->time = time;
	this->tracklet_len++;
...
int STrack::next_id()
{
	thread_local static int _count = 0;
	_count++;
	return _count;
}
...
float STrack::end_time()
{
	return this->time;
}

void STrack::multi_predict(vector<STrack*> &stracks, byte_kalman::KalmanFilter &kalman_filter, float dt)
{
